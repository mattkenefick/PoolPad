...
class DB {
    ...
    function select($table){
        ...
        reset($this->fields);
        foreach($this->fields as $field=>$value){
            $f.= ($f!=""?" and ":"").$field." = '" . mysql_real_escape_string($value) . "'";
        }
        $sql = "SELECT * FROM `$table` WHERE $f";
        $this->query($sql);
        return $this->fetch_row_object();
    }
    ...
    function query($_query){
        ...
        if ($this->result = @mysql_query($_query, $this->link_id)) {
            list($usec, $sec) = explode(" ",microtime());
          ...
          return $_query."<p>".mysql_error($this->link_id);
        }
    }
    ...
    function movenext(){
        $this->col=@mysql_fetch_object($this->result);
